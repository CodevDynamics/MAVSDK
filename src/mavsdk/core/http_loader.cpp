...
HttpLoader::HttpLoader(const std::string& username, const std::string& password)
    : _curl_wrapper(std::make_unique<CurlWrapper>(username, password))
{
    start();
}
...
{
    WorkItem item = UploadItem{target_url, local_path, nullptr};
    bool success = do_item(item);
    return success;
}
...
{
    _work_queue.push_back(
        std::make_shared<WorkItem>(UploadItem{target_url, local_path, progress_callback}));
}
...
bool HttpLoader::do_item(WorkItem& item)
{
    return std::visit(
        overloaded{
            ...
            [&](UploadItem& upload_item) { return do_upload(upload_item); },
        },
        item);
}
...
bool HttpLoader::do_upload(const UploadItem& item)
{
    bool success = _curl_wrapper->upload_file(item.get_target_url(), item.get_local_path(), item.get_progress_callback());
    return success;
}
