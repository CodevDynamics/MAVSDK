...
MavsdkImpl::add_udp_connection(const CliArg::Udp& udp, ForwardingOption forwarding_option)
{
    ...
    } else if(udp.mode == CliArg::Udp::Mode::In && udp.remotes.size() > 0) {
        for(auto& remote : udp.remotes) {
            new_conn->add_remote_to_keep(remote.first, remote.second);
        }
    }
...
Mavsdk::ConnectionHandle MavsdkImpl::add_connection(std::unique_ptr<Connection>&& new_connection, UdpConnection* udp_connection_ptr)
{
    std::lock_guard lock(_mutex);
    auto handle = _connections_handle_factory.create();
    _connections.emplace_back(ConnectionEntry{std::move(new_connection), handle, udp_connection_ptr});
...
std::vector<std::string> MavsdkImpl::get_udp_active_remote_ip()
{
    ...
    for (auto& entry : _connections) {
        if (entry.udp_connection_ptr) {
            auto remotes = entry.udp_connection_ptr->remotes();
            for (auto& remote : remotes) {
                if (time.elapsed_since_s(remote.last_activity) < 5 &&
...
void MavsdkImpl::work_thread()
{
    ...
    while (!_should_exit) {
        ...
        if (time.elapsed_since_s(last_ping_time) >= PING_INTERVAL_S &&
            _configuration.get_component_type() == ComponentType::GroundStation) {
            if (is_any_system_connected()) {
                ping.run_once();
            }
...
void MavsdkImpl::start_sending_heartbeats()
{
    ...
    if (!_configuration.get_disable_send_heartbeats()) {
        std::lock_guard<std::mutex> lock(_heartbeat_mutex);
        call_every_handler.remove(_heartbeat_send_cookie);
